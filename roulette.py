...
#############################################
##### CONVERT BET TO NUM ID FOR $ RETURN ####
def convertBetId(ID):
    ID = str(ID)
    resultId = -1
    if ID == "0" or ID == "00":
    ...
    if betAmount <= int(money) and not betIdBox == "" and int(money) > 0:
        ...
        for x in validId:
            if x == betId:
                ...
                # 30,31 WILL ALWAYS LAND ON 1 [TESTING]
                # ACTUAL WHEEL VALUES (76,380) 2-10 ROTATIONS
...
########################################
#### CREATES BALL WITH RANDOM ATTRIBUTES
...
# Draw labels for the 1-3rd 12's
...
def updateStatLabels(otherWin, oddL, evenL, redL, blackL, greenL):
    ...
    key.setFace("courier")
    key.draw(otherWin)
    # Update stat labels showing odds of each
